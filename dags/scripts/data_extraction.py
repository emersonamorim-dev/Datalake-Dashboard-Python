...
def extract_meteomatics_data():
    ...
    # Iterar pelas regiões e coletar os dados
    for region, coords in coordinates.items():
        # Montar a URL da API com as coordenadas e datas
        ...
        try:
            ...
            if response.status_code == 200:
                # Armazenar os dados da região no dicionário
    ...
    # Gerar um timestamp para nomear o arquivo
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Caminho do arquivo JSON para salvar os dados
    output_path = f"data/raw/processed/data_{timestamp}.json"
